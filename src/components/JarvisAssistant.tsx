...
export default function JarvisAssistant({ context, variant = 'dark' }: JarvisAssistantProps) {
  ...
  const handleCommand = useCallback(async (commandText: string) => {
    ...
    try {
      ...
      if (!res.ok || !command) {
        throw new Error('Jarvis no pudo interpretar el pedido')
      }
        ...
        <div className="jarvis-orb-shell" style={{
          ...
          <div className="jarvis-orb-core" style={{
            ...
            background: status === 'error'
              ? 'radial-gradient(circle at 30% 30%, #fca5a5, #7f1d1d 70%)'
              : status === 'processing'
                ? 'radial-gradient(circle at 30% 30%, #fcd34d, #7c3aed 72%)'
          ...
          <div className="jarvis-orb-panel" style={{
            ...
            <p style={{ margin: '6px 0 0', fontSize: 13, lineHeight: 1.5, color: '#f8fafc' }}>
              {errorText || reply || heardText || 'Decí "Hola Jarvis" y después tu pedido.'}
            </p>
          </div>
        </div>
      )}
